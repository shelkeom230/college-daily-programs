...
def calDiseal(fuel,distance):
    if fuel<=0 or distance<=0:
        return 'Invalid input' 
    else:
        fuel_in_gallons=round(fuel*0.2642,2)
        dist_in_miles=round(distance*0.6214,2)
        
        fuel_per_100=round((fuel*100)/distance,2)
        fuel_consumption=round((dist_in_miles/fuel_in_gallons),2)
        return fuel_consumption

print("enter no. of litres to fill the tank: ")
litres=int(input())
print("enter distance covered by the truck: ")
distance=int(input())    
print(calDiseal(litres,distance))